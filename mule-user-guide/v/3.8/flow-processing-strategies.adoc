...
|custom-queue-store |A custom queue store defined with Spring properties. You will also need to set the system property "mule.queue.objectstoremode=true" for the custom queue store to be used.
|===
